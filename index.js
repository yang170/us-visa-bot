  ...
  try {
    ...
    while (true) {
      ...
      if (!date) {
        ...
        book(sessionHeaders, date, time).then((d) =>
...
async function login() {
  ...
  const anonymousHeaders = await fetch(`${BASE_URI}/users/sign_in`).then(
...
function checkAvailableDate(headers) {
  const desired_start_date = "2024-10-1";
  const desired_end_date = "2024-10-30";
  ...
  const options = fetch(
    ...
    .then((avaliable_dates) =>
      avaliable_dates.length > 0
        ? avaliable_dates.filter(
            (avaliable_date) =>
              Date.parse(avaliable_date.date) >=
                Date.parse(desired_start_date) &&
              Date.parse(avaliable_date.date) <= Date.parse(desired_end_date)
          )
        : []
    );

  return options.length > 0 ? options[0].date : null;
}
